...
**Goal:** Close a Prefill `REVERSE_RECEIVE` completion after Decode proves either that the exact Reverse attempt is terminal or that the attempt lies above the frozen admission activation ceiling, without scheduler-side early release and without same-ID or cross-attempt aliasing.

**Architecture:** Preserve the existing completion barrier and `dispatched` guard. Decode attaches optional exact-attempt terminal proof and frozen admission activation ceiling to `PathAbortNotice`; Prefill derives exact close authority per attempt and converts authorized dispatched attempts into exact worker control terminals. Every Prefill worker consumes its matching binding as failed and reports through the existing `DualPathWorkerMetadata` barrier, after which the existing close action injects `finished_recving` and releases blocks.
...
- ABORT without either exact terminal proof or admission ceiling records the exact admission as invalid. It emits invalid-block control failure only while that key is the current exact admission and its current waiting attempt belongs to the same admission; it does not change or close the completion, stage a worker terminal, or inject `finished_recving`.
...
- Every Decision delivery failure is ambiguous, including `PathDecisionRejectedError(UNKNOWN_REQUEST/STALE_CLOSED/PROTOCOL_ERROR)`. An earlier send may have been accepted while its ACK was lost, followed by registry transition or unregister before a retry receives the typed rejection. Until the protocol carries durable attempt-specific `NEVER_ACCEPTED` evidence or a terminal receipt, rejected, timed-out, cancelled, generic, and untyped delivery failures must not change or close a completion, stage a worker terminal, or inject `finished_recving`. This is a fail-closed availability limitation; this delta does not extend the protocol.
...
## Authoritative follow-on: admission activation ceiling

The exact-terminal implementation exposed one remaining availability gap: an exact proof for attempt 0 cannot and must not close Prefill's current attempt 1, yet Decode will never activate attempt 1 after the admission enters `ACTIVATION_FAILED`.

`.specs/2026-08-24-dualpath-admission-activation-ceiling-design.md` is the authoritative follow-on design and supersedes the exact-proof-only assumptions in Task 3 below:

- Decode freezes an exact-admission publication ceiling after disabling future activation. Publication and freeze share one scheduler serialization domain with a final pre-publication terminal gate. Integer `N` proves attempts `> N` never started; an explicit nested `null` proves no attempt started; an absent ceiling field proves nothing.
- Exact attempt proof and admission ceiling are independent authorities and may coexist. Neither may alias through request id.
- Any accepted ABORT invalidates the whole exact admission. If the ABORT key is still current and the entry-snapshotted waiting attempt belongs to it, Prefill emits current-plan invalid blocks even when the exact proof names a stale attempt.
- Completion closure remains attempt-scoped. An attempt `<= N` without exact proof stays open even when locally undispatched. An authorized undispatched attempt closes in the scheduler; an authorized dispatched attempt uses the real worker terminal and all-worker barrier.
- Decode retains the frozen ceiling with late ABORT endpoint state. Prefill retains the first observed ceiling in an exact-key ledger across request-state release until abort-key unregister; repeated explicit ceilings for one admission must be identical. A conflicting ceiling grants no ceiling authority, while an independently valid co-carried exact proof still applies only to its named attempt.

The old stale-attempt oracle is therefore split in two: stale exact proof alone must not terminalize the current completion, but it still produces current-plan control failure for the same exact admission; adding a ceiling that excludes the current attempt authorizes that attempt's normal undispatched/worker-barrier close path.
...
When a failed `REVERSE_SEND` completion closes, send an ABORT carrying `completion.reverse_attempt_key` as terminalized. For DE_READ activation or refresh failure before a new plan reaches workers, pass the decision's exact attempt id into `_fail_decode_admission()` and emit a terminalized proof for that unstarted attempt. PE_READ and failures without an exact safe attempt send no exact-attempt proof; after the admission becomes terminal they still carry the frozen admission ceiling, typically explicit `null` when no Reverse plan was published. Never recover the exact attempt from current request-id state after the failing operation.

- [ ] **Step 4: Stage exact Prefill worker terminals**

In `_handle_received_peer_abort()` resolve the exact admission and snapshot the current waiting attempt before any close action. Every accepted ABORT records the exact admission invalid; a current exact admission whose current waiting attempt belongs to that admission emits invalid-block control failure independently of which attempt an exact proof names. A proofless ABORT with no admission ceiling does not mutate or close a completion, stage `ReverseReceiveFailureTerminal`, or inject `finished_recving`. For an attempt authorized by an exact proof or by the frozen admission ceiling, call `force_fail_completion()`: undispatched completions close immediately; dispatched open completions stage one exact `ReverseReceiveFailureTerminal` in scheduler-owned pending state. An attempt at or below the ceiling without exact proof remains open even if locally undispatched. Track staged-or-delivered exact attempts until their completion closes so a duplicate ABORT cannot redispatch the terminal after `build_connector_meta()` drains the pending payload but before worker reports return. Retire that dedupe state from both current and stale completion close paths. Existing worker reports then close through `_aggregate_worker_completion_reports()` and `_run_completion_close_action()`.

The exact-attempt terminal helper is reserved for peer ABORT carrying explicit `TERMINALIZED` proof. `_reconcile_prefill_deliveries()` must treat every failure outcome as ambiguous. It may roll back a deferred replacement, restore/dispatch the prior exact binding, mark the admission invalid, and emit invalid-block control failure, but it leaves the completion open with `failed=False`, stages no worker terminal, and injects no `finished_recving`. A typed final rejection describes only the retry that produced it and cannot establish historical non-acceptance.

- [ ] **Step 5: Add barrier and isolation tests**

Cover TP=2, ordinary ABORT without proof, old admission versus same-ID replacement, attempt 0 versus attempt 1, stale exact proof versus admission-wide invalidation, absent/explicit-null/integer ceilings, attempts on both sides of the ceiling, late-context retention, and duplicate/conflicting evidence. Add real-ZMQ channel regressions for proofless→exact→combined ceiling, attempt 0→attempt 1, duplicate exact retry, and unregister/drop/re-register. Add delivery reconciliation regressions that include all three typed rejection statuses in the ambiguous boundary, keep a dispatched completion open without synthetic terminal, restore the exact prior attempt after deferred refresh rollback, and characterize ACK-loss followed by a typed final rejection.

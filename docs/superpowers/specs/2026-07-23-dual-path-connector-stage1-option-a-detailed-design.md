...
- `PE_READ`：KV 从 AscendStore 加载到 Prefill Engine，Prefill 计算未命中的尾部，
  再通过既有 Mooncake Layerwise 正向链路传给 Decode Engine；
...
| `T_PE = R - K_PE` | PE Read 路径中 PE 必须计算并 Forward 的尾部 |

`get_num_new_matched_tokens()` 的返回值必须按 Engine 区分：

| Engine / 路径 | 返回 token 数 | `load_async` | 含义 |
|---|---:|---:|---|
| DE / `PE_READ` | `R - L_DE` | `True` | DE 等待 PE Store + PE compute + Forward |
| DE / `DE_FULL_HIT` | `R - L_DE` | `True` | DE 等待 Store load |
| DE / `DE_PARTIAL_READ` | `R - L_DE` | `True` | DE 等待 Store + Forward 的组合结果 |
...
- PE Read 的 PE Store prefix 是 `[L_PE, K_PE)`，计算和 Forward tail
  是 `[K_PE, R)`。
...
DualPath 至少同时涉及路由请求、两个 Engine 的本地请求、Store lookup
以及 P2P wire。不得把这些 ID 混成一个 `request_id`。
...
- `route_request_id`：PE 和 DE 之间的路径协商主键；
- `*_engine_local_request_id`：只能用于对应 Engine 的
...
- `*_wire_external_id`：只用于对应 P2P channel；
- `transfer_epoch`：防止迟到事件污染复用后的 request ID；
- wire 事件只有在 route、direction、epoch 都匹配时才可接收；
...
作为硬准入条件。rank-level transport arm 发生在 commit 之后；arm 失败使
请求进入 `FINISHED_ERROR`。
...
DE 在 decision 前声明的 matched-token 总量始终是 `R - L_DE`。这不是路径
提交，只表示无论最终由 DE Store、PE Forward 还是二者组合，这些 tokens
都将异步到达 DE。该统一总量允许复用 vLLM 的正常 block allocation 顺序。
...
1. DE 的 `get_num_new_matched_tokens()` 执行 Store probe，得到候选
   `StoreCoverage`；
2. DE 记录 pending candidate，并返回 `(R - L_DE, True)`；
3. Scheduler 分配 DE 正式 blocks，调用 `update_state_after_alloc()`；
4. DualPath 冻结 DE block manifest，但不启动 Store/P2P I/O；
5. 对 `PE_READ`/`DE_PARTIAL_READ`，DE 派发带 `CoverageProposal` 的 PE
   模型请求；PE 此时可得到真实 `L_PE`；
6. 对 `DE_FULL_HIT`，DE 只向 PE control coordinator 发送 proposal，
   不创建 PE 模型请求；
7. PE 根据静态策略、`L_PE`、Store coverage、capability 和 DE block
   manifest 形成 `PathDecisionCommit`；
8. PE 将 commit 发给 DE，DE 持久化后返回 `DecisionAck`；
9. PE/DE Worker 绑定 metadata，并按 rank 发送 `RankArmAck`；
10. 所有必要 rank armed 后才启动 Store load、Reverse 或 Forward。
...
sequenceDiagram
    ...
    DP-->>DS: "(R - L_DE, True)"
    DS->>DP: "update_state_after_alloc(real blocks, R - L_DE)"
    ...
    alt "存在 PE 模型请求"
        PC->>PW: "bind PE metadata"
        PW-->>DP: "RankArmAck"
    end
    DP->>DW: "bind DE metadata and arm"
    DW-->>DP: "RankArmAck"
    DP->>DW: "start committed data operations"
```

`update_state_after_alloc()` 可能被框架对同一请求调用两次；实现必须以
`decision_version + transfer_epoch` 幂等处理，且只在
`num_external_tokens > 0` 时冻结真实 block plan。Store/P2P I/O 仍必须
等待 `PathDecisionCommit + DecisionAck + required RankArmAck`。
...
class DualPathConnector(MooncakeLayerwiseConnector):
    ...
    def save_kv_layer(
        ...
        **kwargs: Any,
        ...
        """在 PE 上发送 tail，在 DE 上维护对应 layer 的 ownership。"""
...
class DualPathConnectorWorker(MooncakeLayerwiseConnectorWorker):
    ...
    def mark_forward_armed(
        self,
        request_key: "TransferFenceKey",
    ) -> None:
        """声明本 rank 的正向接收资源与映射已准备完成。"""

    def on_transfer_event(
        self,
        event: "DualPathTransferEvent",
    ) -> None:
        """校验 route、direction、epoch 后更新状态机。"""
...
class DualPathConnectorWorkerMetadata(KVConnectorWorkerMetadata):
    ...
    def aggregate(
        ...
        statuses 按 (route_request_id, epoch, tp_rank) 合并，冲突即协议错误。
        """
```
...
- Forward 区间为 `[K_DE, R)` 或 PE Read 的 `[K_PE, R)`；
...
1. PE 全部 rank 已绑定 Reverse metadata；
2. Reverse receiver 已 armed；
3. DE 在 `STORE_DONE` 后逐层 Reverse；
4. PE 收齐所有层、所有 TP rank 的 receiver terminal；
5. Worker 发布 PE-local `finished_recving`；
6. Scheduler 使 PE 请求进入模型执行，并安全加入 prefix cache；
7. `wait_for_layer_load(layer)` 仍保留为防御性检查，此时应立即通过；
8. PE compute 与 Forward 继续按层流水。
...
| Reverse layer write | PE receive arm 时 | `REVERSE_LAYER_DONE`/明确失败 |
| PE model compute | `REVERSE_LAYER_DONE` 后 | 该层 compute 完成 |
| Forward layer read | 该层 compute 后、发送前 | receiver terminal/明确失败 |
| Forward layer write | DE receive arm 时 | `FORWARD_LAYER_DONE`/明确失败 |
...
- 每个 rank 对该 ID 只发布一次；
- 不得把 `route_request_id`、wire ID 或 proxy dispatch ID 放入
  `finished_*`。
...
- 每个副作用命令都必须携带 `decision_version` 和 `transfer_epoch`。
...
- `start_store_load()` 每个 epoch 至多执行一次；
- 每个 `(epoch, rank, layer, direction)` 的发送至多启动一次；
...
- `P == 0`、`P == 1`；
- `S_raw > R` 时 clamp；
- Store prefix 与 Forward tail 不重叠；
...
- model-request proposal 与 control commit 使用同一 route/epoch；
...
1. PE Store partial hit + Prefill tail + Forward；
...
10. 三路径 UT；
11. 失败、epoch、rank、ownership、delayed-free UT；
12. NPU E2E；
13. 性能与显存审计。
